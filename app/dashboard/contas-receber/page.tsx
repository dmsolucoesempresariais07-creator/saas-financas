...
export default function ContasReceberPage() {
  ...
  const adicionarConta = async (e: React.FormEvent) => {
    ...
    if (!error) {
      setForm({ descricao: '', valor: '', data_vencimento: '', cliente: '', categoria: '', centro_custo: '', numero_nota: '', observacao: '' })
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <form onSubmit={adicionarConta} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.descricao} onChange={e => setForm({...form, descricao: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm" placeholder="Ex: Servico de consultoria" required />
              </div>
              <div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        {conta.status !== 'recebido' && (
                          <button onClick={() => marcarStatus(conta.id, 'recebido')} className="text-green-600 hover:underline text-xs">Recebido</button>
                        )}
