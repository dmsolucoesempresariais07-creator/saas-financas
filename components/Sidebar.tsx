...
const icons = {
  ...
  dre: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
  ...
  geral: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>,
  categorias: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>,
  plano: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>,
  mensal: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
  diario: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
}
...
const menuItems = [
  ...
  { label: 'Contas a Pagar', icon: 'pagar', href: '/dashboard/contas-pagar' },
  {
    label: 'Fluxo de Caixa', icon: 'fluxo', href: '/dashboard/fluxo-caixa', sub: [
      { label: 'Mensal', href: '/dashboard/fluxo-caixa', icon: 'mensal' },
      { label: 'Diário', href: '/dashboard/fluxo-caixa-diario', icon: 'diario' },
    ]
  },
  { label: 'DRE', icon: 'dre', href: '/dashboard/dre', separador: true },
  { label: 'Relatórios', icon: 'relatorios', href: '/dashboard/relatorios' },
  {
    label: 'Configurações', icon: 'config', href: '/dashboard/configuracoes', separador: true, sub: [
      { label: 'Geral', href: '/dashboard/configuracoes', icon: 'geral' },
      { label: 'Categorias Plano de Contas', href: '/dashboard/configuracoes/categorias', icon: 'categorias' },
      { label: 'Plano de Contas', href: '/dashboard/configuracoes/plano-de-contas', icon: 'plano' },
    ]
  },
      ...
      <div>
        ...
        <button
          ...
          className={`flex items-center justify-between w-full px-4 py-2.5 text-sm transition-all duration-150 ${
            isAtivo && !temSub ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'
          } ${recolhido ? 'justify-center px-0' : ''}`}
        >
          <div className={`flex items-center gap-3 ${recolhido ? 'justify-center w-full' : ''}`}>
            <span className={isAtivo && !temSub ? 'text-white' : 'text-gray-400'}>
              {icons[item.icon as keyof typeof icons]}
            </span>
            {!recolhido && <span className="font-medium">{item.label}</span>}
          </div>
          {temSub && !recolhido && (
            <span className={`transition-transform duration-200 ${subEstaAberto ? 'rotate-180' : ''} text-gray-500`}>
              {icons.chevron}
            </span>
          )}
        </button>
          ...
          <div className="bg-gray-900 py-1">
            {item.sub.map((s: any) => (
              <button
                ...
                className={`flex items-center w-full pl-10 pr-4 py-2 text-xs transition gap-2 ${
